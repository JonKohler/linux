 ...
 *
 * Note: Callers must ensure that the ptr_ring is not currently full
 * before calling this function.
 *
 * Note: __ptr_ring_discard_one invalidates entries in reverse order.
 * As a result, after inserting a new entry, the next entry (rather than
 * the current one) may be zeroed out first. This behavior must be
 * considered to avoid false positives when checking for fullness after
 * the next insertion.
 */
